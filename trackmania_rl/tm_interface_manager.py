...
class TMInterfaceManager:
    ...
    def rollout(self, exploration_policy, stats_tracker):
        ...
        cpcount = 0
        current_zone_idx = 0
        current_zone_center = self.zone_centers[0, :]
        next_zone_center = self.zone_centers[1, :]
        prev_sim_state_position = self.zone_centers[0, :]
        ...
        while not (this_rollout_is_finished and time.perf_counter_ns() > do_not_exit_main_loop_before_time):
            ...
            if (msgtype & 0xFF != 14) and (((msgtype & 0xFF00) == 0) or ignore_message0):
                ...
                if (
                    ...
                    if current_zone_idx == len(self.zone_centers) - 1:
                        ...
                        if np.linalg.norm(next_zone_center - sim_state_position) <= np.linalg.norm(
                            ...
                            current_zone_center = self.zone_centers[current_zone_idx]
                            next_zone_center = self.zone_centers[current_zone_idx + 1]
                        else:
            ...
            if msgtype == MessageType.S_SHUTDOWN:
                ...
                if (
                    ...
                    stats_tracker["race_time_for_ratio"].append(misc.max_overall_duration_ms)
