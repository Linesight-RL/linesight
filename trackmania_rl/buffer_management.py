...
def fill_buffer_from_rollout_with_n_steps_rule(
    ...
    for i in range(n_frames - 1):
        ...
        mini_race_range = range(
            max(0, current_zone_idx + 2 - n_zone_centers_in_inputs), 1 + min(current_zone_idx, len(zone_centers) - n_zone_centers_in_inputs)
        )
        ...
        for first_zone_idx_in_input in random.sample(mini_race_range, min(len(mini_race_range), misc.sample_n_mini_races)):
            ...
            assert state_zone_center_coordinates_in_car_reference_system.shape == (n_zone_centers_in_inputs, 3)
