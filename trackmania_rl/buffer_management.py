    ...
    for i in range(n_frames - 1):
        ...
        for first_zone_idx_in_input in random.sample(
            ...
            last_zone_idx_in_input = (
                first_zone_idx_in_input + n_zone_centers_in_inputs - 1
            )  # included
            assert last_zone_idx_in_input < len(zone_centers)

            time_mini_race_start_ms = rollout_results["zone_entrance_time_ms"][
                first_zone_idx_in_input
            ]
            current_overall_time_ms = (
                i * misc.ms_per_tm_engine_step * misc.tm_engine_step_per_action
            )
            ...
            for n_steps in range(n_steps_max, 0, -1):
                ...
                if (
                    ...
                    + (n_steps - 1)
                    * misc.ms_per_tm_engine_step
                    * misc.tm_engine_step_per_action
                    < misc.max_minirace_duration_ms  # Previous frame was not "race timeout"
                ):
                    mini_race_finished = (
                        rollout_results["current_zone_idx"][i + n_steps]
                        == last_zone_idx_in_input
                    )
            ...
            if mini_race_finished:
                ...
                gammas[-1] *= rollout_results["fraction_time_in_previous_zone"][
                    i + n_steps
                ]
            ...
            reward += (
                np.sum(
                    gammas
                    * np.array(
                        rollout_results["display_speed"][i + 1 : i + 1 + n_steps]
                    )
                )
            ...
            state_zone_center_coordinates_in_car_reference_system = (
                car_orientation.T.dot(
                    (
                        zone_centers[
                            first_zone_idx_in_input : last_zone_idx_in_input + 1, :
                        ]
                        - car_position
                    ).T
                ).T
            )  # (n_zone_centers_in_inputs, 3)
            state_y_map_vector_in_car_reference_system = car_orientation.T.dot(
                np.array([0, 1, 0])
            )
            ...
            state_float = np.hstack(
                (
                    ...
                    current_zone_idx
                    >= np.arange(first_zone_idx_in_input + 1, last_zone_idx_in_input),
                )
            ).astype(
                np.float32
            )  # TODO : cache this so that state_float and next_state_float can share memory for different experiences
            ...
            if not done:
                ...
                next_state_zone_center_coordinates_in_car_reference_system = (
                    next_car_orientation.T.dot(
                        (
                            zone_centers[
                                first_zone_idx_in_input : last_zone_idx_in_input + 1, :
                            ]
                ...
                next_state_y_map_vector_in_car_reference_system = (
                    next_car_orientation.T.dot(np.array([0, 1, 0]))
                )
                ...
                next_state_float = np.hstack(
                    (
                        ...
                        rollout_results["current_zone_idx"][i + n_steps]
                        >= np.arange(
                            first_zone_idx_in_input + 1, last_zone_idx_in_input
                        ),
                    )
                ).astype(np.float32)
            else:
