    ...
    def train_on_batch(self, buffer: ExperienceReplayInterface):
        ...
        with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
            ...
            with torch.no_grad():
                ...
                outputs_model_next, _ = self.model(next_state_img_tensor, next_state_float_tensor, self.iqn_n, tau=None)
                # outputs_model_next  : (batch_size*iqn_n,n_actions)
                outputs_model_next = outputs_model_next.reshape([self.iqn_n, self.batch_size, self.model.n_actions])
                ...
                outputs_model2_next = torch.gather(outputs_model2_next, 1, actions_model_next)
                # outputs_model2_next : (batch_size*iqn_n, 1)
                outputs_target = torch.where(done, rewards, rewards + pow(self.gamma, self.n_steps) * outputs_model2_next)
