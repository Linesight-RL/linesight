    ...
    def train_on_batch(self, buffer: ExperienceReplayInterface):
        ...
        with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
            ...
            with torch.no_grad():
                ...
                actions = actions[:, None].repeat(
                    [self.iqn_n, 1]
                )  # (batch_size*iqn_n, 1)
                self.model.reset_noise()
                outputs_model_next, _ = self.model(
                    next_state_img_tensor, next_state_float_tensor, self.iqn_n, tau=None
                )
                # outputs_model_next  : (batch_size*iqn_n,n_actions)
                outputs_model_next = outputs_model_next.reshape(
                    [self.iqn_n, self.batch_size, self.model.n_actions]
                )
                # outputs_model_next (iqn_n, batch_size, n_actions)
                outputs_model_next = torch.mean(outputs_model_next, dim=0)
                ...
                actions_model_next = actions_model_next[:, None].repeat(
                    [self.iqn_n, 1]
                )  # (iqn_n * batch_size, 1)
                self.model2.reset_noise()
                outputs_model2_next, tau = self.model2(
                    next_state_img_tensor, next_state_float_tensor, self.iqn_n, tau=None
                )
                # outputs_model2_next  : (batch_size*iqn_n,n_actions)
                # tau : (batch_size*iqn_n, 1)
                outputs_model2_next = torch.gather(
                    outputs_model2_next, 1, actions_model_next
                )
                # outputs_model2_next : (batch_size*iqn_n, 1)
                outputs_target = torch.where(
                    done,
                    rewards,
                    rewards + pow(self.gamma, self.n_steps) * outputs_model2_next,
                )
                # outputs_target : (batch_size * iqn_n, 1)
                outputs_target = outputs_target.reshape(
            ...
            outputs, _ = self.model(
                state_img_tensor, state_float_tensor, self.iqn_n, tau=tau
            )
            # outputs  : (batch_size*iqn_n,n_actions)
            mean_q_value = torch.mean(outputs, dim=0).detach().cpu()
            outputs = torch.gather(outputs, 1, actions)
            # (batch_size*iqn_n, 1)
            ...
            loss = (
                torch.where(TD_Error < 0, 1 - tau, tau) * loss / self.iqn_kappa
            )  # pinball loss
